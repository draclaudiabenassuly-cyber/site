...
export function PublicFooter({ content, mediaRevision = 0 }: { content: SiteContent; mediaRevision?: number }) {
  return <footer className="public-editorial-footer"><div><img src={publicMediaSrc(content.siteFooterLogo, mediaRevision)} alt="Cláudia Benassuly" /><p>{content.slogan}</p></div><div className="public-editorial-footer-links"><Link href="/historia">A história</Link><Link href="/propostas">Propostas</Link><Link href="/compromissos">Compromissos</Link><Link href="/noticias">Notícias</Link><Link href="/galeria">Galeria</Link><Link href="/santinho">Santinho</Link></div><div><strong>Fale com a campanha</strong><a href={campaignWhatsAppLink(content.whatsappMessageMore || whatsappMessages.more, content.whatsappNumber)} target="_blank" rel="noreferrer">WhatsApp ↗</a><small>© 2026 Cláudia Benassuly · CNPJ 68.553.373/0001-23</small></div></footer>;
}

export function PublicFrame({ content, mediaRevision = 0, loading = false, eyebrow, title, emphasis, description, children }: { content: SiteContent; mediaRevision?: number; loading?: boolean; eyebrow: string; title: string; emphasis: string; description: string; children: ReactNode }) {
  return <main className={`public-editorial-page${loading ? " is-hydrating" : ""}`} aria-busy={loading}><PublicHeader content={content} mediaRevision={mediaRevision} /><section className="public-editorial-hero"><p className="eyebrow"><span /> {eyebrow}</p><h1>{title} <em>{emphasis}</em></h1><p>{description}</p></section>{children}<CampaignContactTools content={content} /><PublicFooter content={content} mediaRevision={mediaRevision} /></main>;
}

export function HistoryPage() {
  const { content, loading, mediaRevision } = useCmsData();
  return <PublicFrame content={content} loading={loading} mediaRevision={mediaRevision} eyebrow={content.storyEyebrow} title={content.storyTitle} emphasis={content.storyTitleEm} description={content.storyLead}><section className="public-editorial-split"><div className="public-editorial-image"><img src={publicMediaSrc(content.storyImage, mediaRevision)} alt="Retrato de Cláudia Benassuly" /></div><article><p className="large-lede">{content.storyLead}</p><p>{content.storyBody}</p><p>{content.storyExtra}</p><div className="public-editorial-quote">“{content.quote}<br /><em>{content.quoteEm}</em>”</div></article></section></PublicFrame>;
}
...
export function ProposalsPage() {
  ...
  return <PublicFrame content={content} loading={loading} mediaRevision={mediaRevision} eyebrow={content.proposalsEyebrow} title={content.proposalsTitle} emphasis={content.proposalsTitleEm} description={content.proposalsSupport}><section className="public-proposal-grid">{cards.map((card, index) => <article className="public-proposal-card" key={card.number}><span>{card.number}</span><img src={publicMediaSrc(images[index % images.length], mediaRevision)} alt="" /><h2>{card.title}</h2><p>{card.copy}</p><small>{card.tag}</small></article>)}</section><section className="public-editorial-callout"><span>Na linguagem da rua</span><strong>{content.streetLanguage}</strong></section></PublicFrame>;
}

export function CommitmentsPage() {
  const { content, agenda, loading, mediaRevision } = useCmsData();
  return <PublicFrame content={content} loading={loading} mediaRevision={mediaRevision} eyebrow={content.agendaEyebrow} title={content.agendaTitle} emphasis={content.agendaTitleEm} description={content.agendaDescription}><section className="public-commitments-list">{agenda.length ? agenda.map((item) => <article className="public-commitment-card" id={`event-${item.id ?? item.day}`} key={item.id ?? item.date}><div className={`public-agenda-date ${item.tone}`}><strong>{item.day}</strong><span>{item.month}</span></div><div><span>{item.date} · {item.location}</span><h2>{item.title}</h2><p>{item.detail}</p></div><a href={campaignWhatsAppLink(content.whatsappMessageCommitment || whatsappMessages.commitment, content.whatsappNumber)} target="_blank" rel="noreferrer">Falar com a campanha ↗</a></article>) : <div className="public-empty-state">Novos compromissos serão publicados em breve.</div>}</section></PublicFrame>;
}

export function NewsPage() {
  const { content, news, loading, mediaRevision } = useCmsData();
  const [active, setActive] = useState<NewsItem | null>(null);
  return <PublicFrame content={content} loading={loading} mediaRevision={mediaRevision} eyebrow={content.newsEyebrow} title={content.newsTitle} emphasis={content.newsTitleEm} description={content.newsDescription}><><section className="public-news-grid">{news.map((item, index) => <article className="public-news-card" id={`news-${item.id ?? index}`} key={item.id ?? item.title} role="button" tabIndex={0} aria-label={`Abrir notícia: ${item.title}`} onClick={() => setActive(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setActive(item); } }}><div className="public-news-image"><img src={publicMediaSrc(item.image, mediaRevision)} alt="" /></div><div><span>{item.category} · {item.publishedAt}</span><h2>{item.title}</h2><p>{item.excerpt}</p><small>{item.readTime}</small></div></article>)}</section>{active && <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={() => setActive(null)}><article className="post-modal" onClick={(event) => event.stopPropagation()}><button type="button" className="modal-close" onClick={() => setActive(null)} aria-label="Fechar notícia">×</button><img src={publicMediaSrc(active.image, mediaRevision)} alt="" /><div><span>{active.category}</span><h2>{active.title}</h2><p>{active.excerpt}</p><small>{active.readTime}</small></div></article></div>}</></PublicFrame>;
}

export function GalleryPage() {
  const { content, albums: cmsAlbums, photos: cmsPhotos, loading, mediaRevision } = useCmsData();
